...
class ClassErrorMeter(meter.Meter):
    ...
    def add(self, output, target):
        ...
        pred = torch.from_numpy(output).topk(maxk, 1, True, True)[1].numpy()  # pred is the topk largest values' index
    ...
    def value(self, k=-1):
        if k != -1:
            ...
            return {k_: self.value(k_) for k_ in self.topk}

    def __str__(self):
        rst = 'Acc [ ' if self.accuracy else 'Error rate [ '
        for k in self.topk:
            rst += 'top{}: {}\t'.format(k, self.value(k))
        return rst[:-1] + ']'
